...
struct sync_bug_detect : public FunctionPass {
  ...
  using AccessMap = map<string, vector<pair<pair<string, string>, string>>>;
  ...
  bool redundantcheck(Function &F) {
    errs() << F.getName() << "\n";
    // errs()<<regions.size()<< "-----------------------------------\n";
    regions.clear();
    AccessMap current;
        ...
        if (auto *load = dyn_cast<LoadInst>(&I)) {
          ...
          if (ptr) {
            if (auto *gep = dyn_cast<GetElementPtrInst>(ptr)) {
              ...
              type = "read";
              current[baseName].push_back({{a, b}, type});
          ...
          if (ptr) {
            if (auto *gep = dyn_cast<GetElementPtrInst>(ptr)) {
              ...
              type = "write";
              current[baseName].push_back({{a, b}, type});
              errs() << "Write from " << baseName << "[" << a << "]\n";
              errs() << "Write from " << baseName << "[" << b << "]\n";
            }
          }
        }
      }
...
static RegisterPass<sync_bug_detect> X(
    "sync_bug_detect",
